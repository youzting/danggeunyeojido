...
## 7. 실제 공개 웹 데이터 공급자 연결 중 지역 해석 오류

### 증상

`daangn` 프로필로 실제 공개 웹 데이터 공급자를 연결했을 때 일부 거점에서 결과가 0건으로 나왔다.

로그에는 다음 문제가 보였다.

```text
당근 HTTP 응답 실패. status=500
당근 검색 응답 본문 없음
```

### 원인

지역명 자동 해석 API가 `대전 서구` 같은 시군구 단위 입력에서 중복 지명을 잘못 선택했다.

예를 들어 `서구`만 fallback으로 사용하면 실제 의도와 다른 지역의 동네가 선택될 수 있다.

또한 일부 동네 id는 검색 JSON 라우트에서 500을 반환했다.

### 해결

내부 `Region` id 기준으로 실제 공급자용 대표 동네를 고정 매핑했다.
...
이후 `GET /api/search/nationwide?keyword=맥북&maxStops=3` 기준 실제 공개 데이터 6건을 가져오는 것을 확인했다.
